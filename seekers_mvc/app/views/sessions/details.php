            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <small class="text-muted">
                        Criada por <?= htmlspecialchars($sessao['criador']) ?> em <?= date('d/m/Y H:i', strtotime($sessao['criado_em'])) ?>
                    </small>
                </div>
                <div class="card-body">
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6">
                            <?php if(isset($_SESSION['usuarioLogado']) && $sessao['status'] == 'aberta'): ?>
                                ...
                                <?php if($_SESSION['usuarioLogado'] == $sessao['criador_id'] || $participa): ?>
                                    <a href="<?= BASE_URL ?>/chat_sessao?id=<?= $sessao['id'] ?>" class="btn btn-success mb-2">
                                        💬 Entrar no Chat
                                    </a><br>
                                <?php endif; ?>
                                ...
                                <?php elseif($_SESSION['usuarioLogado'] == $sessao['criador_id']): ?>
                                    <a href="<?= BASE_URL ?>/editar_sessao?id=<?= $sessao['id'] ?>" class="btn btn-warning mb-2">
                                        Editar Sessão
                                    </a><br>
                                    <a href="<?= BASE_URL ?>/gerenciar_solicitacoes?id=<?= $sessao['id'] ?>" class="btn btn-info mb-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <?php if(empty($sessoes_relacionadas)): ?>
                        ...
                        <a href="<?= BASE_URL ?>/criar_sessao" class="btn btn-outline-primary btn-sm">Criar Nova Sessão</a>
                    <?php else: ?>
                                ...
                                <div class="card bg-dark border-primary">
                                    <div class="card-body py-2">
                                        ...
                                        <div class="d-flex justify-content-between">
                                            <a href="<?= BASE_URL ?>/sessao_detalhes?id=<?= $sessao_rel['id'] ?>" class="btn btn-primary btn-sm">Ver</a>
                        ...
                        <?php endforeach; ?>
                        <a href="<?= BASE_URL ?>/sessoes" class="btn btn-outline-primary btn-sm w-100">Ver Todas as Sessões</a>
                    <?php endif; ?>
                </div>
            </div>
...
function participarSessao(sessaoId) {
    ...
    if(confirm('Deseja participar desta sessão? O criador será notificado.')) {
        $.post('<?= BASE_URL ?>/public/ajax/solicitar_participacao.php', {
            sessao_id: sessaoId
        }, function(data) {
            alert(data.message);
        }, 'json');
